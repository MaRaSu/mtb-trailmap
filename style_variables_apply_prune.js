 ...
 * - Copies style_base_v3.json to style_[variant]_v3.json.
 * - Replaces all @tags in the copy.
 * - NEW: Removes any JSON object that has the key-value pair
 *        "trailmap:[variant]": false, then saves in place.
 */
...
// --- NEW STEP: prune objects with "trailmap:[variant]": false ---
const variantKey = `trailmap:${VARIANT}`;

// Utility to decide if an object should be removed
function shouldRemoveObject(obj) {
  return obj && typeof obj === 'object' &&
         Object.prototype.hasOwnProperty.call(obj, variantKey) &&
         obj[variantKey] === false; // NOTE: boolean false, not string "false"
}

// Recursively prune matching objects from arrays and object properties.
// Returns number of objects removed.
function pruneInPlace(node) {
  let removed = 0;

  if (Array.isArray(node)) {
    // Walk backward so splice doesn't shift remaining items
    for (let i = node.length - 1; i >= 0; i--) {
      const el = node[i];
      if (el && typeof el === 'object') {
        if (shouldRemoveObject(el)) {
          node.splice(i, 1);
          removed++;
        } else {
          removed += pruneInPlace(el);
        }
      }
    }
    return removed;
  ...
  if (node && typeof node === 'object') {
    // If the current object itself matches, we do NOT delete it here,
    // because that would require the caller to drop their reference.
    // (The root object cannot be deleted; arrays & parent objects handle child removal.)
    for (const [k, v] of Object.entries(node)) {
      if (v && typeof v === 'object') {
        if (shouldRemoveObject(v)) {
          delete node[k];
          removed++;
        } else {
          removed += pruneInPlace(v);
        }
      }
    }
  }
...
try {
  ...
  // Helpful diagnostics if replacements resulted in invalid JSON
...
prunedCount = pruneInPlace(parsed);
...
try {
  ...
  console.log(`✅ Pruned ${prunedCount} object(s) with "${variantKey}": false.`);
